...
export async function processBranding(): Promise<void> {
  ...
  // Adaptive SVG masters (site favicon, icon, logo, banner). Kept as-is.
  ...
  copyInto(
    resolve(BRANDING_DIR, 'purview-logo-horizontal.svg'),
    resolve(PUBLIC_BRANDING_DIR, 'logo-horizontal.svg'),
  );
  copyInto(resolve(BRANDING_DIR, 'purview-banner.svg'), resolve(PUBLIC_BRANDING_DIR, 'banner.svg'));
